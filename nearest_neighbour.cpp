...
/*********************
This is the main prog of nearest neighbor finder.
...
Each person is then randomly generated by choosing a random first name a random family name
from the set, and giving a random valid age and a random valid location.
The location is scoped in the Greater London area, which is roughly between 51.295449 and 51.709192 in latitude,
and between -0.471357 and 0.157610 in longitude. I also chose the age to be between 10 and 90.
It can be easily extended to support other areas by modifying the location range. But for the current implementation, please 
only search for people within this area, as otherwise the results would be meaningless.

Several optional parameters can be provided for the main prog,
including the size of the randomly generated population, choosing for fast launching or not,
...
Noting that the construction might take from a few seconds to tens of minutes, given the required size of data set and CPU.

Then it will prompt the user for providing his/her age, latitude and longitude,
and print a list of 10 'closest' people on the standard output.
The asking_and_answering process will loop forever until the user terminate it by pressing q when prompted.

Noting that the term 'closest' is determined by the weights provided when starting the program. By default, it will set to be age:geogaphy = 1:10. That means the closeness in distance 
